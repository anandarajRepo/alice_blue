...
class AliceBlue:
    ...
    @staticmethod
    def login_and_get_sessionID(username, password, twoFA, app_id, api_secret, totp_key):
        ...
        # Web Login Totp
        totp = str(pyotp.TOTP(totp_key).now())
        data = {"userId": username, 'tOtp': totp}
        header['Authorization'] = f"Bearer {username} {auth_us}"
        r = requests.post(AliceBlue.__urls["verifyTotp"], headers=header, data=json.dumps(data))
        logging.info(f"Totp Login response {r.text}")

        # Web Login Api Key
        userSessionID = r.json()['userSessionID']
        header['Authorization'] = f"Bearer {username} {userSessionID}"
        r = requests.post(AliceBlue.__urls["getApiKey"], headers=header)
        ...
        for exch in body:
            if (exch != "contract_date"):
                for scrip in body[exch]:
                    ...
                    if ("expiry_date" in scrip and scrip['expiry_date'] is not None):
